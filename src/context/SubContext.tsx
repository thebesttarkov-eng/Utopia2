...
interface SubContextValue extends SubState {
  activate: (plan: Plan, devices: number) => void
  deactivate: () => void
}
...
const INITIAL: SubState = { active: false, plan: null, devices: 1, expiresAt: null, daysLeft: 0 }
...
function loadState(): SubState {
  try {
    ...
    return { active: parsed.active, plan: parsed.plan, devices: parsed.devices, expiresAt, daysLeft }
  } catch {
    return INITIAL
  }
}
...
export function SubProvider({ children }: { children: ReactNode }) {
  ...
  function activate(plan: Plan, devices: number) {
    const expiresAt = new Date()
    expiresAt.setMonth(expiresAt.getMonth() + PLAN_MONTHS[plan])
    const daysLeft = Math.ceil((expiresAt.getTime() - Date.now()) / 86_400_000)
    setState({ active: true, plan, devices, expiresAt, daysLeft })
  }
